...
# Production-Ready & Spark 4 Safe Version
# ==========================================================
...
spark = (
    ...
    .master("local[*]")
    .config("spark.sql.ansi.enabled", "false")  # disable strict ANSI
    .getOrCreate()
)
...
df_clean = (
    df.dropDuplicates()
      .dropna(subset=["transaction_id", "customer_id", "price", "quantity"])
...
df_clean = df_clean.withColumn(
    "transaction_date",
    expr("try_to_date(transaction_date, 'dd-MM-yyyy')")
)
